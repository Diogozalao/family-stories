...
Texto: Ollama (local) $\rightarrow$ Groq $\rightarrow$ Gemini; Visão: Gemini & Apenas uma API
comercial (GPT-4/Claude) & Privacidade e custo nulo em local; texto na nuvem
no Groq e visão no Gemini — quotas gratuitas separadas, sem competir entre si. \\
...
depende de conectividade). Como o ambiente de produção gratuito não dispõe
de \textsc{gpu} para executar modelos locais, a geração de \textbf{texto}
na nuvem recorre, por esta ordem, ao \textbf{Groq} (modelos abertos
\emph{Llama~3.3~70B}, camada gratuita generosa) e, só se este falhar, ao
\textbf{Google Gemini~Flash}. Esta repartição é deliberada: a quota
gratuita do Gemini fica \textbf{reservada para a análise visual} das
fotografias (M1), que usa sempre o \textbf{Gemini Vision} (\emph{2.5-Flash};
o \emph{1.5} foi descontinuado) por ser a única via com capacidade
multimodal madura na cadeia. Assim, repartindo o texto (Groq) e a visão
(Gemini) por dois serviços gratuitos distintos, evita-se esgotar a quota
diária de um único fornecedor. Em ambiente local, o texto é servido
primeiro pelo Ollama. Esta arquitetura é detalhada no
...
fornecedor em uso. O \textbf{Ollama} (local) mantém-se como primeira opção
para texto quando há \textsc{gpu}/\textsc{cpu} disponível, e o Gemini
permanece como rede de segurança final para texto — garantindo que uma
falha pontual de um fornecedor nunca impede a geração.
