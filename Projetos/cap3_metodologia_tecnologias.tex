...
Ollama local + Gemini + Groq (cadeia de \emph{fallback}) & Apenas uma API
comercial (GPT-4/Claude) & Privacidade e custo nulo em local; \emph{fallback}
robusto na nuvem (Gemini) e rede de segurança grátis (Groq). \\
...
depende de conectividade). Como \emph{fallback} principal recorre-se ao
\textbf{Google Gemini~Flash} (\emph{2.0} para texto, \emph{2.5} para
visão; o \emph{1.5} foi descontinuado), pois o ambiente de produção gratuito não dispõe de \textsc{gpu}
para executar modelos locais. Para a análise visual das fotografias (M1)
usa-se sempre o \textbf{Gemini Vision}, por ser a única via com
capacidade multimodal madura na cadeia. Como \emph{segunda} rede de
segurança \textbf{apenas para texto}, foi ainda integrado o \textbf{Groq}
(modelos abertos em \emph{cloud}, camada gratuita generosa), acionado só
quando o Gemini falha. Esta arquitetura é detalhada no
\Cref{cap:implementacao}.

\paragraph{Um fornecedor, não vários.} O sistema foi propositadamente
desenhado para suportar \emph{múltiplos fornecedores de IA} — em teoria,
poder-se-ia atribuir cada tarefa ao fornecedor que melhor a serve (um para
visão, outro para texto, etc.). Optou-se, contudo, por concentrar a
geração num \textbf{único fornecedor principal} (Gemini), por três
motivos: \emph{(i)} simplicidade — cada fornecedor adicional implica mais
uma credencial a gerir, mais código específico e mais pontos de falha;
\emph{(ii)} suficiência — os modelos \emph{Flash} cobrem texto \emph{e}
visão com qualidade adequada a custo de cêntimos; e \emph{(iii)} controlo
de custos — um único ponto de faturação é mais fácil de vigiar (quota
diária + orçamento). O \textbf{Ollama} (local) e o \textbf{Groq} (texto)
não contradizem esta escolha: não são fornecedores ``paralelos'', mas sim
\emph{camadas de resiliência} que apenas entram quando o caminho preferido
falha.
