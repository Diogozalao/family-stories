...
M3 a usar a totalidade dos factos como contexto. Na prática, uma importação
condicional do \texttt{chromadb} deteta a ausência da biblioteca e comuta o
\texttt{RAGSystem} para esse modo \emph{stub}, sem que o resto do sistema o
note. O sistema mantém-se \emph{funcional}, perdendo apenas o passo de
\emph{retrieval}.
...
executor \emph{in-process} usa um \emph{thread pool} limitado a um
trabalhador — suficiente para um sistema single-tenant e prudente face aos
$\approx$512~MB de memória do plano gratuito; cada \emph{thread} corre o seu
próprio \texttt{asyncio.run}, para não bloquear o \emph{loop} principal do
FastAPI.
