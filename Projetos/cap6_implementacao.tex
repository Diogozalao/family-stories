...
$\rightarrow$ \textsc{exif} $\rightarrow$ análise visual $\rightarrow$
\textsc{ocr} $\rightarrow$ carregamento para o \emph{Storage}, persistindo
no fim um registo \texttt{MediaFile}.
...
Gerações longas podem correr em segundo plano via Celery + Redis, com o
\emph{frontend} a acompanhar o estado por \emph{polling}. Uma decisão de
robustez relevante é o \textbf{\emph{sweep} de tarefas órfãs} no arranque:
como um \emph{worker} Celery não sobrevive a um reinício do \emph{backend},
qualquer tarefa que ficasse \texttt{pending}/\texttt{running} de um
arranque anterior é, por definição, órfã, sendo marcada como falhada
(\Cref{lst:sweep}). Em ambientes sem \emph{worker} (como o \emph{deploy}
na nuvem), a configuração \texttt{CELERY\_ENABLED=False} rebaixa
transparentemente qualquer chamada \texttt{background} para execução
síncrona.
