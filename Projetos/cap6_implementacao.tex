...
\subsection{Narração e sincronização áudio-vídeo}

A narração é gerada por \textbf{síntese de voz} na língua em que a
história foi escrita (capturada em \texttt{Story.language} no momento da
geração), garantindo coerência mesmo que o utilizador altere depois o
idioma da interface. O áudio é cuidadosamente alinhado: silêncio durante o
cartão de título e, em seguida, a narração; se esta termina antes dos
visuais, é preenchida com silêncio; se é mais longa, é cortada — mantendo
a duração A/V consistente. Há ainda suporte opcional para música de fundo
em volume reduzido. A \Cref{fig:frame_video} mostra um fotograma típico do
resultado.
