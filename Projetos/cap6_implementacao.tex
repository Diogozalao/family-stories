...
def generate(self, prompt: str, max_tokens: int = 1500) -> str:
    # Texto: Groq (70B, rapido) -> Ollama (offline/local) -> Gemini.
    # Groq primeiro porque o Ollama 3B local em CPU e demasiado lento.
    if self._groq_ok:                              # 1. Groq (nuvem, rapido)
    ...
    return self._gemini_generate(prompt, max_tokens)  # 3. ultimo recurso
...
A geração raramente acerta à primeira no registo pretendido. Para o
suportar sem obrigar a recomeçar, o leitor de histórias oferece uma
...
A \textbf{navegação} cobre os dez destinos da aplicação (Início, Biblioteca,
Família, Linha Temporal, Projetos, Gerar, Histórias, Vídeos, Tarefas e
Definições), e cada história exibe um \emph{badge} com o \textbf{projeto} a que
pertence (com ligação direta), clarificando a organização do acervo. A
