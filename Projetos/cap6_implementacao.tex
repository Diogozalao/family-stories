...
O \texttt{LLMClient} privilegia a inferência local via \emph{Ollama}
(Llama~3.2~3B); se este não estiver disponível, recorre ao \emph{Gemini
1.5~Flash}. O \Cref{lst:llm} mostra o coração desta lógica: se a inferência
local falhar \emph{durante} a geração, há ainda um \emph{fallback} em
tempo de execução; se ambos falharem, é lançada uma exceção que faz a
tarefa ser marcada como falhada — evitando guardar uma narrativa falsa.
