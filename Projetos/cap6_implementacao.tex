...
Para não exceder a memória da instância gratuita, a re-análise processa as
fotografias em \textbf{pequenos lotes} por pedido, prosseguindo até estarem
todas tratadas --- em vez de carregar a biblioteca inteira de uma só vez, o
que esgotava a instância.
...
por gerações, casais lado a lado e nós coloridos por sexo; as ligações
genealógicas usam um \textbf{barramento} partilhado --- uma descida a
partir do casal, uma barra de irmãos e uma ligação curta a cada filho ---,
evitando linhas sobrepostas) alternável com a lista, e um \textbf{editor
manual} que permite criar/editar pessoas e ligações à mão
(\emph{pai}/\emph{mãe}/\emph{filho}/\emph{irmão}/\emph{casado(a)/parceiro(a)})
--- incluindo um assistente de \textbf{criação rápida de árvore}
pré-estruturada (eu, pais, avós, irmãos) --- sem depender de um GEDCOM. Após cada edição, o grafo
...
objetiva de aderência ao PT-PT (\Cref{sec:qualidade}). O pós-processador foi
ainda estendido com a correção de \textbf{concordância de género} num conjunto
curado de substantivos (por exemplo, ``o~lã''\,$\rightarrow$\,``a~lã''), a
\textbf{remoção de parágrafos repetidos} --- que o modelo por vezes produz
para preencher extensões longas com poucos factos --- e o \textbf{corte de uma
frase final incompleta}, deixada por um limite de \emph{tokens} atingido a
meio.

\textbf{Regras de estilo e integração das imagens.} Dois blocos adicionais
foram anexados ao \emph{prompt} para elevar a qualidade literária. As
\texttt{STYLE\_RULES} combatem os defeitos típicos do texto gerado: proíbem a
repetição de nomes próprios (impondo pronomes, determinantes e perífrases ---
``o avô'', ``ela''), exigem variedade de verbos e de conectores, e reforçam a
colocação pronominal europeia. As \texttt{MEDIA\_GROUNDING} instruem o modelo
a \emph{preparar} a entrada de cada fotografia --- situando o espaço, o lugar
e as pessoas antes de descrever a cena --- e a fazer transições suaves entre
momentos, para que a imagem surja em contexto e não de forma abrupta.

\textbf{Controlo de extensão.} O assistente permite escolher a duração da
narrativa em quatro níveis --- \emph{curta} ($\approx$1\,min), \emph{média}
($\approx$2--3\,min), \emph{longa} ($\approx$4--5\,min) e \emph{épica}
($\approx$6--8\,min). Cada nível traduz-se numa instrução de extensão de
prioridade máxima (que sobrepõe o número de parágrafos do \emph{template}) e
num limite de \emph{tokens} calibrado para a duração pretendida
($\approx$140~palavras por minuto de narração), evitando que o texto dispare
ou fique cortado. Para não sacrificar a qualidade, o \emph{prompt} estabelece
que a \textbf{não-repetição tem prioridade sobre o comprimento}: se os factos
não sustentarem a extensão pedida, o modelo escreve um texto mais curto mas
completo, em vez de encher com repetições.
...
equitativa. Numa narrativa \textbf{longa ilustrada por poucas fotografias},
os parágrafos que sobram --- aqueles em que a narração já não se refere a
uma imagem em concreto --- são preenchidos \textbf{alternando} ciclicamente
as fotografias disponíveis (evitando repetir a da cena anterior), de modo
que o vídeo continua a mudar de imagem em vez de congelar numa só; cada
fotografia surge primeiro no parágrafo que a descreve e só depois é
reaproveitada. Assim, nenhuma cena fica sem fotografia. Toda a etapa é
...
para igualar a duração A/V). A \Cref{fig:frame_video}
(Anexo~\ref{ap:capturas}) mostra um fotograma típico do resultado.
