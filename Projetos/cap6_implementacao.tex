...
\emph{fallback} de três níveis (Ollama $\rightarrow$ Gemini $\rightarrow$
Groq) em tempo de execução (\texttt{llm\_client.py}).},label={lst:llm}]
def generate(self, prompt: str, max_tokens: int = 1500) -> str:
    if self._ollama_ok:                            # 1. local (privacidade)
        return self._ollama_generate(prompt, max_tokens)
    return self._gemini_generate(prompt, max_tokens)  # 2. nuvem

def _ollama_generate(self, prompt: str, max_tokens: int) -> str:
    try:
        response = ollama_client.generate(
            model=settings.OLLAMA_MODEL, prompt=prompt, options={...})
        return response.response.strip()
    except Exception as e:                         # Ollama falhou em runtime
        self._setup_gemini()
        return self._gemini_generate(prompt, max_tokens, _ollama_error=str(e))

def _gemini_generate(self, prompt, max_tokens, _ollama_error=None) -> str:
    try:
        return self._gemini.generate_content(prompt, ...).text  # 2. Gemini
    except Exception as exc:
        if settings.GROQ_API_KEY:                  # 3. Groq (so' texto)
            return self._groq_generate(prompt, max_tokens)
        raise LLMUnavailableError(...)             # todos falharam -> falha
\end{lstlisting}
...
ambientes restritos, \Cref{sec:limitacoes}). O \texttt{M4Processor} orquestra a
