...
\emph{Ollama} (Llama~3.2~3B); se esta não estiver disponível --- como
sucede no alojamento gratuito, sem \textsc{gpu} --- recorre ao \emph{Groq}
(modelos abertos \emph{Llama~3.3~70B}, camada gratuita), que é o
\textbf{motor de texto na nuvem}; e só se o Groq falhar entra o
\emph{Gemini~Flash} como última rede de segurança. Esta ordem é
\textbf{deliberada}: a quota gratuita do Gemini fica \textbf{reservada para
a análise visual} (M1), que se mantém \emph{Gemini-only} por o Groq não
oferecer visão equivalente. Reparte-se assim o texto (Groq) e a visão
(Gemini) por dois serviços gratuitos distintos, evitando que compitam pela
mesma quota diária. O \Cref{lst:llm} mostra o coração desta lógica: se um
nível falhar \emph{durante} a geração, há \emph{fallback} em tempo de
execução; só quando \emph{todos} falham é lançada uma exceção que marca a
...
são parametrizáveis por variável de ambiente, permitindo aliviar a memória em
ambientes restritos, \Cref{sec:limitacoes}; em produção usa-se $854\times480$
a 20~fps, pois a 720p o \emph{render} excedia os 512~MB e deixava o vídeo
preso indefinidamente em processamento). O \texttt{M4Processor} orquestra a
seleção cronológica das fotografias, a síntese de voz da narrativa e a montagem
do vídeo, mantendo apenas espaço de trabalho temporário no disco (as
...
a instância acordada durante o \emph{render} (evitando o problema das tarefas de
segundo plano abandonadas no plano gratuito).
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Cartões de título e de fim} de abertura e fecho, e
  \textbf{legendas} (\emph{lower thirds}) com data e local de cada
  fotografia.
