...
efeito \emph{Ken~Burns}, narração por síntese de voz e legendas. A
narrativa é segmentada em \emph{cenas} ligadas a fotografias concretas, o
que permite ao M4 sincronizar cada imagem com a sua narração.
...
para autenticação, base de dados e armazenamento). Adota uma estratégia
de inteligência artificial híbrida — inferência local privilegiada via
\emph{Ollama} (Llama~3.2) com recurso automático ao \emph{Google~Gemini} —
e técnicas de engenharia de robustez (degradação graciosa do \textsc{rag},
\emph{fallbacks} de modelo e rebaixamento síncrono) que mantêm o sistema
operacional mesmo em ambientes com dependências reduzidas. Os resultados
...
documentais coerentes a partir de um acervo familiar heterogéneo.
...
into a documentary video with a Ken~Burns effect, text-to-speech
...
authentication, database and storage). It adopts a hybrid AI strategy —
local-first inference via Ollama (Llama~3.2) with automatic fallback to
Google~Gemini — and robustness engineering techniques (graceful RAG
degradation, model fallbacks and synchronous downgrade) that keep the
system operational even in dependency-constrained environments. The
