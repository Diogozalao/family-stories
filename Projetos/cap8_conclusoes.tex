...
\emph{slideshow} num verdadeiro documentário); a \textbf{geração
assíncrona} — quando não há \emph{worker} Celery, as tarefas longas correm
num executor \emph{in-process}, devolvendo a resposta de imediato e
atualizando o estado por \emph{polling}; uma \textbf{árvore familiar
...
\begin{itemize}[leftmargin=*]
  ...
  gratuito.} A montagem com MoviePy/FFmpeg pode exceder os 512~MB de RAM do
  plano gratuito do Render, que termina a instância (\emph{out-of-memory}) a
  meio do \emph{render}. Mitiga-se de duas formas: \emph{(i)} a resolução e os
  \emph{fps} são parametrizáveis por ambiente, permitindo baixá-los para caber;
  e \emph{(ii)} como o MP4 final é persistido no \emph{Storage} (Supabase) e a
  sua reprodução é leve (apenas servir o ficheiro), os vídeos podem ser
  \textbf{pré-gerados em ambiente local} --- sem o limite de memória e
  escrevendo no mesmo \emph{backend} --- e depois reproduzidos no site sem
  qualquer \emph{render} em tempo real. A reprodução nunca é afetada; apenas a
  geração em tempo real no plano gratuito.
