...
class GeminiAnalyzer:
    """
    Usa Gemini Vision (free tier) para analisar fotografias:
    - Descreve o conteúdo
    - Conta pessoas
    - Identifica contexto/local
    - Sugere como a foto se encaixa numa narrativa
    """
    ...
    def _setup(self):
        if not settings.GEMINI_API_KEY or settings.GEMINI_API_KEY.startswith("coloca"):
            log.warning("gemini_not_configured", msg="Gemini API Key não configurada")
            return
        try:
    ...
    def analyze(self, file_path: Path) -> dict:
        ...
        try:
            ...
            # Reduz se muito grande (poupar tokens do free tier)
            img.thumbnail((1024, 1024))

            response = self.model.generate_content([ANALYSIS_PROMPT, img])
            text = response.text.strip()

            # Limpa markdown code blocks se existirem
            if text.startswith("```"):
                text = text.split("```")[1]
            ...
            log.error("gemini_json_error", error=str(e))
            return self._fallback()
        except Exception as e:
    ...
    def _fallback(self) -> dict:
        """Retorna estrutura vazia se Gemini não disponível."""
        return {
            "ai_description":    None,
